...
//
// Same twelve functions, but only two of them carry data. See the note in
// CMakeLists.txt for why: the other ten vocabularies belong to text encoders
...
std::string not_built_in(const char * encoder) {
    __android_log_print(
        ...
        "no built-in %s vocabulary: this build carries CLIP and T5 only", encoder);
    return {};
}
...
std::string load_qwen2_merges() { return not_built_in("Qwen 2"); }
